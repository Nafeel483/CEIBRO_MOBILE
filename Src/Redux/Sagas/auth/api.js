...
// verifyUserApi
export const verifyUserApi = (user) => {
  return axios.post(`${CONSTANTS.BASE_URL}/auth/verify-email?otp=${user.otp}`).then(response => response);
}

// resetUserPasswordApi
export function* resetUserPasswordApi(user) {
  console.log(' resetUserPasswordApi ---Data ', user)
  const data = new URLSearchParams();
  data.append('password', user.password)
  ...
  const response = yield fetch(`${CONSTANTS.BASE_URL}/auth/reset-password?otp=${user.otp}`, opt);
  const message = yield response
